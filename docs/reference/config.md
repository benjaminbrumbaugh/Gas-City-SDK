...
| `delete_after_close` | string |  |  | DeleteAfterClose deletes matching GC-owned beads after they have been closed for this duration. Accepts Go duration syntax plus whole-day "d" units, e.g. "7d" or "1d12h". ApplyBeadPolicyDefaults fills in a non-empty default for recognized policy types (order_tracking: "7d"), so this field is populated after config load even when the city.toml omits it. The opt-in "closed_durable" policy has no default: when set, the controller ages out closed non-ephemeral beads past this duration that have no parent or dependency link to non-closed work (dry-run unless Enforce or GC_CLOSED_BEAD_RETENTION_ENFORCE=1). |
| `enforce` | boolean |  |  | Enforce switches the closed_durable sweep from its dry-run advisory to actual deletion. It is ignored by every other policy. The environment variable GC_CLOSED_BEAD_RETENTION_ENFORCE=1 is an equivalent override for operators who cannot edit city.toml; the config field is the durable choice because a supervisor reinstall regenerates the service environment from an allowlist and drops ad hoc variables. |
