...
// fakeBDFailsInProgressServesReady is a `bd` stand-in for the fall-through case
// proper: the crash-recovery tier's read fails the way an unreadable store makes
// it fail, and the tier behind it answers normally. Logs every invocation to
// $FAKE_BD_LOG for the same reason fakeBDFails does — the single-store tiers
// send bd's stderr to /dev/null, so without the log there is no evidence the
// injected failure was ever reached.
const fakeBDFailsInProgressServesReady = `#!/bin/sh
[ -n "$FAKE_BD_LOG" ] && printf '%s\n' "$*" >> "$FAKE_BD_LOG"
case "$1" in
  list) printf 'bd: store unreadable\n' >&2; exit 1 ;;
  ready) printf '[{"id":"sk-1","status":"open"}]'; exit 0 ;;
  *) printf '[]'; exit 0 ;;
esac
`
...
// still swallows a failing tier's error and falls through to the next one. This
...
//
// REWRITTEN for gc-mece, and the rewrite is the point. This case used to inject
// a `bd` that failed EVERY subcommand and then assert the script still exited 0
// printing "[]". That fixture could not tell "fell through and found nothing"
// apart from "fell through and everything died", and it was pinning the second
// one — the exact fail-open where a wholly unreadable store answered `gc hook`
// with an empty array and a silent stderr, indistinguishable from a genuinely
// empty hook. The fall-through contract this case exists to protect is that a
// failing tier does not abort the script and a LATER tier can still serve work,
// so that is what it now injects and asserts. The all-tiers-failed case is
// TestEffectiveWorkQueryRefusesEmptyAnswerWhenStoreUnreadable, which requires
...
func TestSingleStoreWorkQueryKeepsItsFallThrough(t *testing.T) {
	...
	res := runGeneratedQueryWithBD(t, command, map[string]string{
		...
		"GC_SESSION_ORIGIN": "ephemeral",
		"FAKE_BD_LOG":       bdLog,
	}, fakeGCReadyFails, fakeBDFailsInProgressServesReady)
	if res.exit != 0 {
		t.Fatalf("the single-store work_query exited %d when an early tier failed and a later one could serve; a failing tier must not abort the script (stderr=%q)", res.exit, res.stderr)
	}
	if !strings.Contains(res.stdout, "sk-1") {
		t.Errorf("single-store work_query stdout = %q, want the bead the tier behind the failing one served", res.stdout)
	}
	invocations, err := os.ReadFile(bdLog)
	if err != nil {
		t.Fatalf("reading bd invocation log: %v", err)
	}
	if !strings.Contains(string(invocations), "list") {
		t.Fatalf("the failing `bd list` tier was never invoked (log=%q); a case whose failure injection is not reached proves nothing about the fall-through", invocations)
	}
	if !strings.Contains(string(invocations), "ready") {
		t.Fatalf("the tier behind the failing one was never reached (log=%q)", invocations)
	}
}
