...
func subscriptionMetadata(record SubscriptionRecord, payload string) map[string]string {
	return map[string]string{
		subscriptionRecordMetadata: subscriptionRecordPayload(record, payload),
...
func subscriptionRecordPayload(record SubscriptionRecord, payload string) string {
	// Create cannot know the store-minted bead ID until after Create. The ID is
	// assigned by decodeSubscriptionBead from the enclosing bead identity.
	if record.ID == "" {
		return payload
	}
