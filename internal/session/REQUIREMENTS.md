...
| SESSION-START-009 | Operator-guarded stale close | `gc session close-stale` accepts only an exact persisted session bead ID and requires exact revision, status `open`, lifecycle state, a valid RFC3339 `held_until` that is still future, and an operator reason. It refuses a session key, pending create claim, observed runtime, blocked issue, capability gap, or any row revision change. Terminal metadata and status close commit in one native Dolt transaction; no unconditional fallback is allowed. `--check` validates the same predicates without writing. | `internal/session/guarded_stale_close_test.go`; `internal/beads/native_dolt_store_close_conditional_internal_test.go`; `cmd/gc/session_close_stale_test.go` |
| SESSION-START-010 | Clear stale city-stop latch | `gc session clear-city-stop` accepts only an exact persisted session bead ID and exact future `held_until`. It requires an open asleep/suspended session with `sleep_reason=city-stop`, no session key/create claim or observed runtime, and atomically value-CASes only `sleep_reason` from `city-stop` to empty. A value race or capability gap writes nothing; `--check` is read-only. | `internal/session/city_stop_clear_test.go`; `cmd/gc/session_clear_city_stop_test.go` |
| SESSION-START-011 | Operator hold provenance survives drain completion | Both acknowledged and controller-completed drains preserve the `sleep_intent=user-hold` marker alongside `held_until`; transient idle-stop intent still clears on ordinary sleep. This prevents an explicit suspend from taking on the bounded-heartbeat shape after drain. | `internal/session/lifecycle_transition_test.go` (`TestDrainCompletionPatchesPreserveOperatorHoldProvenance`); `gc-j95` |
| SESSION-START-012 | Empty resume identity recovery | When startup dies after the persisted `session_key` clears but the launch command still carries a resume shape, normal and runtime-only starts strip that shape and retry once as fresh. If the command has no resume shape, they do not relaunch it and return the original startup error. | `internal/session/chat_empty_session_key_test.go` |
...
| SESSION-RECON-015 | Alive provider-account fence detection | The reconciler probes a bounded round-robin subset of alive sessions independently of opt-in progress-stall policy. A stale, unattended pane with the strict usage-limit choice modal records a session quarantine and, when a stable account identity exists, durably fences the launched provider account until the parsed reset (or the conservative fallback), even after the source session closes. New standalone observations without a stable provider-account identity stay explicitly session-local and are never widened to the legacy global fence; pre-identity rows may still migrate conservatively. Account identities are opaque city-keyed digests of provider family and credential/account material; role commands, models, endpoints, and credential destination names do not split aliases, while distinct literal account material remains separate. Unpinned sessions persist a fresh-conversation handoff before stopping and can wake after a healthy-account repoint; operator-pinned named sessions retain their established abrupt-kill protection and remain visibly fenced for operator action. Fence-store errors, probe errors, runtime-incarnation mismatches, and attached sessions fail closed without destructive mutation. Runtimes lacking atomic conditional stop retain a visible session quarantine and durable account fence but no destructive restart handoff, so siblings stay blocked while the original instance remains pinned for operator/runtime remediation. Repoint metadata stays quarantined until an instance-bound stop succeeds. Any durable keyed fence or session-attribution history prevents silent identity-key regeneration or missing-digest attestation; corrupt labeled fence rows fail closed, identify the operator repair target, and are atomically auto-closed in bounded batches only after a conservative safety interval. Expired immutable fence rows are closed in bounded batches. Direct identity-aware starts enforce the same durable fence and atomically promote their write-ahead attribution on success. | `cmd/gc/provider_fence_identity.go`; `cmd/gc/session_usage_limit_account_fence_test.go`; `cmd/gc/session_usage_limit_modal.go`; `cmd/gc/session_usage_limit_modal_test.go`; `cmd/gc/session_reconciler_test.go` (`TestReconcileSessionBeads_DesiredFastPathBoundsProviderFenceObservation`); `internal/runtime/dialog.go`; `internal/runtime/dialog_test.go`; `internal/session/provider_fence_store.go`; `internal/session/provider_fence_store_test.go`; `internal/session/lifecycle_exits.go` (`ProviderFencePatch`); `internal/session/lifecycle_exits_test.go` |
| SESSION-RECON-016 | Account-fenced launch attribution under races and restart recovery | API-created and API-reconstructed provider sessions derive a launch-only fence identity from the same exact current effective launch environment as CLI/worker paths; persisted identities are used only when no current-environment resolver is available, and account material is neither retained as separate identity state nor persisted. Legacy global fences and exact account fences use one matching rule across direct manager creates/resumes and reconciler starts. Launch ownership is staged by value-CAS inside the serialized manager start boundary; after acquiring the durable claim, a caller freshly re-reads the session and converges without restaging when an earlier cross-process caller has already started and committed the live runtime, even if that earlier claim has since been released. Commit attribution prefers the staged/already-running identity so a losing concurrent caller cannot relabel the winner. Before a conditional stop, every destructive handoff field must still match its tick-snapshot preimage and the whole handoff commits under one revision CAS; any changed precondition fails closed without stopping or overwriting. After a successful stop, restart-marker consumption is also conditional and never replays the destructive tick snapshot over newer metadata. A refused conditional stop restores destructive handoff fields only while each value still matches the handoff, preserving concurrent newer state and failing closed when conditional writes are unavailable. Every labeled fence row, including invalid non-open status, enters the bounded operator-visible corruption policy; valid closed history is inactive. The city key's non-secret SHA-256 digest is independently anchored in durable beads state before keyed history is admitted, so coherent local key-plus-digest replacement is rejected without persisting key or credential material. | `internal/api/handler_sessions_test.go` (`TestHandleProviderSessionCreateAttestsCurrentProviderFenceIdentity`, `TestHumaProviderSessionCreateAttestsCurrentProviderFenceIdentity`); `internal/api/worker_factory_test.go` (`TestWorkerFactorySessionByIDRecomputesProviderFenceIdentityOnResume`); `cmd/gc/provider_fence_exact_review_regression_test.go`; `internal/session/provider_fence_exact_review_regression_test.go`; `cmd/gc/worker_handle.go`; `internal/session/chat.go`; `internal/session/store.go`; `internal/session/provider_fence_store.go` |
...
| SESSION-WORK-005 | Controller claim-lease maintenance | From one complete current-cycle session census, the controller renews claims only for active/awake owners with an exact assignee, persisted session name, instance token, and claim session ID. Missing or partial ownership evidence and unreadable suspension state fail closed. Native reclaim is allowed only for terminally closed owner sessions, operates only on configured active store scopes, and runs with bounded native calls outside the controller tick. | `cmd/gc/claim_lease_reconciler_test.go`; `internal/beads/bdstore_lease_test.go` |
