...
| SESSION-RECON-016 | Configured recovery responder | When `[recovery_responder]` is absent, provider/session health handling is byte-identical and creates no work. When configured, only exact durable high-confidence terminal-provider or usage-limit-modal classifications open a durable incident and bounded `held_until`; ambiguous health does nothing. A short-timeout Wayfinder request may recommend only one still-untried configured target. Unavailable, failing, or invalid advice falls back in configured order. The controller creates at most one ordinary routed task at a time through the work store, never starts a responder session directly, and persists incident, attempt, attempted-target, cooldown, active-work, and outcome state on the source session so restarts adopt rather than duplicate work. No credential, environment, account identity, or HMAC fence participates. | `internal/worker/recovery_responder_test.go`; `internal/worker/recovery_responder_wayfinder_test.go`; `internal/config/recovery_responder_test.go`; `cmd/gc/recovery_responder_controller.go`; `gc-k043y` |
