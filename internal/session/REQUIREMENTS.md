...
| SESSION-RECON-015 | Alive provider-fence detection | The reconciler probes a bounded round-robin subset of alive sessions independently of opt-in progress-stall policy. A stale, unattended pane with a strict usage-limit choice modal, or another high-confidence provider rate-limit screen carrying a parseable reset deadline, is scored unhealthy and quarantined until the parsed reset (or the conservative fallback). Unpinned sessions are restarted fresh without selecting a paid-spend option; operator-pinned named sessions retain their established abrupt-kill protection and remain visibly fenced for operator action. Probe errors, runtime-incarnation mismatches, and attached sessions fail closed without destructive mutation. | `cmd/gc/session_usage_limit_modal.go`; `cmd/gc/session_usage_limit_modal_test.go`; `cmd/gc/session_reconciler_test.go` (`TestReconcileSessionBeads_DesiredFastPathBoundsProviderFenceObservation`); `internal/runtime/dialog.go`; `internal/runtime/dialog_test.go`; `internal/session/lifecycle_exits.go` (`ProviderFencePatch`); `internal/session/lifecycle_exits_test.go` |
| SESSION-RECON-016 | Configured recovery responder | When `[recovery_responder]` is absent, provider/session health handling is byte-identical and creates no work. When configured, only exact durable high-confidence terminal-provider tuples (matching terminal error and health reason, unhealthy, drainable) or strict usage-limit-modal classifications open a durable incident and bounded `held_until`; ambiguous or stale mixed health does nothing. A short-timeout Wayfinder request may recommend only one still-untried configured target, and a controller tick starts at most one advisory action. Its regular template is opened by descriptor-relative, no-follow traversal from a pinned city root and its strict typed routing/v3 request/response remains bounded to 128 candidates/targets and 1 MiB; request evidence must exist, remain fresh after `now_unix`, prove an enabled and authorized entitlement, and bind the selected model, opaque entitlement scope, target, and adapter. The responder sends only evidence and targets for surviving candidates, binds returned evidence refs and advisory times to that request, and rejects invalid, expired, unavailable, or close-failing advice before falling back in configured order. Before creating, every responder censuses nonclosed recovery work by source session, then acquires a full revision-CAS lease transition, recensuses, computes the deterministic incident-attempt identity before advice, and invokes a dedicated idempotent create-or-adopt capability. Production stores derive a valid store-owned work ID from that operation identity; exact retries adopt, a persisted planned tuple adopts its still-open deterministic attempt and advances to `attempt_closed` only when no active attempt remains, and conflicting tuples fail, so concurrent processes, expired-lease takeovers, and recurrences create one ordinary work item. The ordinary task is written through the work-class store, never starts a responder session directly, and incident, attempt, attempted-target, cooldown, active-work, and outcome state remain on the source session. No credential, environment, prose, secret, or HMAC fence participates in routing. | `internal/worker/recovery_responder_test.go`; `internal/worker/recovery_responder_wayfinder_test.go`; `internal/beads/deterministic_create_test.go`; `internal/session/recovery_state_test.go`; `internal/config/recovery_responder_test.go`; `cmd/gc/recovery_responder_controller_test.go`; `gc-k043y` |
| SESSION-RECON-017 | Recovery verification is recurrence-safe | Lease acquisition rejects a closed source both before its revision-CAS and after authoritative readback; otherwise it advances the source row revision and returns the only snapshot authorized for responder writes. Every recovery branch, including detected, active, attempt-closed, exhausted, adoption, creation completion, and verification, commits the complete recovery lifecycle, generic hold, and exact deadline-valued ownership marker in one full revision-CAS; stores without that capability fail closed. A successor takeover therefore fences every stale mutation and stale release. Verification atomically clears an exactly recovery-owned generic hold and matching health markers with the verified outcome; when an operator replaced the generic hold it preserves that hold and retires only stale recovery ownership. | `internal/worker/recovery_responder_test.go` (`TestRecoveryResponderLeaseTakeoverDuringDecisionCreatesOneAttempt`, `TestRecoveryResponderCreationCannotOverwriteConcurrentVerification`, `TestRecoveryResponderAdoptionCannotOverwriteConcurrentVerification`, `TestRecoveryResponderVerificationPreservesOperatorModifiedHold`, `TestRecoveryResponderVerificationPreservesUnrelatedOperatorHealthReason`); `internal/session/recovery_state_test.go`; `internal/session/store.go`; `gc-k043y` |
