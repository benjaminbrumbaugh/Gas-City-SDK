...
| SESSION-RECON-015 | Alive provider-account fence detection | The reconciler probes a bounded round-robin subset of alive sessions independently of opt-in progress-stall policy. A stale, unattended pane with a strict usage-limit choice modal, or another high-confidence provider rate-limit screen carrying a parseable reset deadline, durably fences the launched provider account until the parsed reset (or the conservative fallback), even after the source session closes. Account identities are opaque city-keyed digests of provider family and credential/account material; role commands, models, endpoints, and credential destination names do not split aliases, while distinct literal account material remains separate. Unpinned sessions persist a fresh-conversation handoff before stopping and can wake after a healthy-account repoint; operator-pinned named sessions retain their established abrupt-kill protection and remain visibly fenced for operator action. Fence-store errors, probe errors, runtime-incarnation mismatches, and attached sessions fail closed without destructive mutation. | `cmd/gc/provider_fence_identity.go`; `cmd/gc/session_usage_limit_account_fence_test.go`; `cmd/gc/session_usage_limit_modal.go`; `cmd/gc/session_usage_limit_modal_test.go`; `cmd/gc/session_reconciler_test.go` (`TestReconcileSessionBeads_DesiredFastPathBoundsProviderFenceObservation`); `internal/runtime/dialog.go`; `internal/runtime/dialog_test.go`; `internal/session/provider_fence_store.go`; `internal/session/provider_fence_store_test.go`; `internal/session/lifecycle_exits.go` (`ProviderFencePatch`); `internal/session/lifecycle_exits_test.go` |
