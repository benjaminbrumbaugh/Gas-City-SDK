...
const (
	metadataRequest   = "hca.request"
	metadataState     = "hca.state"
	metadataAttempt   = "hca.attempt"
	metadataClaimedBy = "hca.claimed_by"
	metadataClaimedAt = "hca.claimed_at"
	metadataDelivered = "hca.delivered_at"
	metadataError     = "hca.error"
)

var (
	// ErrInvalidInput indicates a malformed or unauthorized HCA operation.
	ErrInvalidInput = errors.New("hca invalid input")
	// ErrNotFound indicates that a request record does not exist.
	ErrNotFound = errors.New("hca request not found")
	// ErrNotQueued indicates that a request is not available for claiming.
	ErrNotQueued = errors.New("hca request is not queued")
	// ErrExpired indicates that a request passed its expiry deadline.
	ErrExpired = errors.New("hca request expired")
	// ErrCancelled distinguishes an operator cancellation.
	ErrCancelled = errors.New("hca request cancelled") //nolint:misspell // public wire spelling
	// ErrUnavailable indicates that no adapter can currently deliver the request.
	ErrUnavailable = errors.New("hca adapter unavailable")
	// ErrPrematureCompletion indicates a transport falsely claimed execution completion.
	ErrPrematureCompletion = errors.New("hca adapter reported completion at delivery boundary")
)
...
func (s *Service) Enqueue(ctx context.Context, input RequestInput) (RequestRecord, error) {
	...
	if request.IdempotencyKey != "" {
		...
		if listErr != nil {
			return RequestRecord{}, fmt.Errorf("list hca requests for idempotency: %w", listErr)
		}
		for _, item := range items {
			if strings.TrimSpace(item.Metadata["hca.idempotency_key"]) != request.IdempotencyKey {
				continue
			}
	...
	if err != nil {
		return RequestRecord{}, fmt.Errorf("encode hca request: %w", err)
	}
	created, err := s.store.Create(beads.Bead{
		Title:       fmt.Sprintf("HCA request: %s", request.Reason),
		Type:        "message",
		...
		Metadata: map[string]string{
			...
			"hca.idempotency_key":   request.IdempotencyKey,
			"hca.content_retention": string(request.ContentRetention),
			metadataState:           string(StateQueued),
			metadataAttempt:         "0",
			"hca.logical_role":      request.Target.LogicalRole,
			"hca.target_id":         request.Target.TargetID,
			"hca.config_revision":   fmt.Sprintf("%d", request.Target.ConfigRevision),
		},
	})
	if err != nil {
		return RequestRecord{}, fmt.Errorf("create hca request: %w", err)
	}
...
func (s *Service) List(ctx context.Context, states ...DeliveryState) ([]RequestRecord, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("list hca requests: %w", err)
	}
...
func (s *Service) Claim(ctx context.Context, id, worker string, now time.Time) (RequestRecord, error) {
	...
	if err := s.store.Update(record.ID, beads.UpdateOpts{
		...
		return RequestRecord{}, fmt.Errorf("claim hca request %s: %w", record.ID, err)
	}
	return record, nil
}

// Complete records the adapter's delivery result. A queued/running receipt is
// deliberately not terminal: transport acceptance is not HCA execution.
...
// RecordResponse records an execution outcome returned by the HCA. This is a
...
func (s *Service) scrubContent(id string, now time.Time) error {
	...
	payload, err := json.Marshal(record.Request)
	if err != nil {
		return fmt.Errorf("encode scrubbed hca request %s: %w", id, err)
	}
	description := "[ephemeral HCA content released]"
	return s.store.Update(id, beads.UpdateOpts{
		Description: &description,
		Metadata: map[string]string{
			metadataRequest:        string(payload),
			"hca.content_released": zeroTime(now).UTC().Format(time.RFC3339Nano),
		},
	})
}
...
func decodeRecord(item beads.Bead) (RequestRecord, error) {
	...
	if err := json.Unmarshal([]byte(item.Metadata[metadataRequest]), &request); err != nil {
		return RequestRecord{}, fmt.Errorf("decode hca request %s: %w", item.ID, err)
	}
