...
// HCA over HTTP. Authentication is supplied at runtime; it is never persisted
...
func (a *HTTPAdapter) Deliver(ctx context.Context, request Request) (DeliveryReceipt, error) {
	...
	if err != nil {
		return DeliveryReceipt{RequestID: request.RequestID, State: StateFailed}, fmt.Errorf("marshal hca request: %w", err)
	}
	httpRequest, err := http.NewRequestWithContext(ctx, http.MethodPost, a.callbackURL, bytes.NewReader(body))
	if err != nil {
		return DeliveryReceipt{RequestID: request.RequestID, State: StateFailed}, fmt.Errorf("create hca request: %w", err)
	}
	httpRequest.Header.Set("Content-Type", "application/json")
	httpRequest.Header.Set("X-HCA-Request-ID", request.RequestID)
	...
	if err != nil {
		return DeliveryReceipt{RequestID: request.RequestID, State: StateFailed}, fmt.Errorf("deliver hca request: %w", err)
	}
	defer response.Body.Close() //nolint:errcheck
	responseBody, err := io.ReadAll(io.LimitReader(response.Body, 1<<20))
	if err != nil {
		return DeliveryReceipt{RequestID: request.RequestID, State: StateFailed}, fmt.Errorf("read hca response: %w", err)
	}
