...
var bootstrapPolicy = Ledger{
	...
	Medium: []MediumOwner{
		{
			PackageDir:      "internal/runtime/acp",
			PackageName:     "acp",
			Owner:           "TestTransferSessionNameOwnerProtectsLiveChildAfterLauncherCrash",
			Resources:       []Resource{ResourceSubprocess},
			OwnerBead:       "gc-k043y",
			Invariant:       "ACP child-ownership transfer is a checked Medium subprocess owner",
			ResourceOwner:   "the one long-lived child process is confined to the ownership-transfer test and killed and waited by test cleanup",
		...
		},
		{
			PackageDir:      "internal/runtime/acp",
			PackageName:     "acp",
			Owner:           "TestReservationLeaseClosesPreTransferLauncherCrashWindow",
			Resources:       []Resource{ResourceSubprocess},
			OwnerBead:       "gc-k043y",
			Invariant:       "ACP inherited-lease crash-window proof is a checked Medium subprocess owner",
			ResourceOwner:   "the one long-lived child process is confined to the inherited-lease test and killed and waited by test cleanup",
			MigrationTarget: "P0.4b",
			Expires:         "2026-10-01",
		},
		{
			PackageDir:      "internal/runtime/acp",
			PackageName:     "acp",
			Owner:           "TestGenerationCleanupDoesNotUnlinkSuccessorSocket",
			Resources:       []Resource{ResourceNetListen},
			OwnerBead:       "gc-k043y",
			Invariant:       "ACP successor-socket generation cleanup is a checked Medium listener owner",
			ResourceOwner:   "both Unix stream listeners are confined to the generation-cleanup test and closed before test exit",
			MigrationTarget: "P0.4c-listener",
			Expires:         "2026-10-01",
		},
		{
			...
			PackageName:     "session",
			Owner:           "TestProviderFenceAccountLockSerializesAcrossProcesses",
			Resources:       []Resource{ResourceSubprocess},
			OwnerBead:       "gc-k043y",
			Invariant:       "provider-account cross-process exclusion is a checked Medium subprocess owner",
			ResourceOwner:   "the one re-executed test helper is confined to the cross-process lock test, released over a pipe, and synchronously waited",
			MigrationTarget: "P0.4b",
			Expires:         "2026-10-01",
		},
		{
