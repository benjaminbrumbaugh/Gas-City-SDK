...
func (s *Server) handleSessionCreate(w http.ResponseWriter, r *http.Request) {
	...
	resolvedCfg, err := resolvedSessionConfigForProvider(s.state.CityPath(), configuredWorkspaceSessionEnv(s.state.Config()), alias, createCtx.ExplicitName, template, title, transport, extraMeta, resolved, command, workDir, mcpServers, store.Store)
	if err != nil {
		s.idem.unreserve(idemKey)
		writeSessionManagerError(w, err)
		return
...
func (s *Server) createProviderSession(w http.ResponseWriter, r *http.Request, store beads.SessionStore, body sessionCreateRequest, providerName, idemKey, bodyHash string) {
	...
	resolvedCfg, err := resolvedSessionConfigForProvider(s.state.CityPath(), configuredWorkspaceSessionEnv(s.state.Config()), alias, "", template, title, transport, extraMeta, resolved, command, workDir, mcpServers, store.Store)
	if err != nil {
		s.idem.unreserve(idemKey)
		writeSessionManagerError(w, err)
		return
