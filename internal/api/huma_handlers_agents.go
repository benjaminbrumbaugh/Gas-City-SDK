...
func (s *Server) humaHandleAgentList(ctx context.Context, input *AgentListInput) (*ListOutput[agentResponse], error) {
	...
	// request. The indexes are complementary: the first preserves the bounded
	// O(stores) ordinary-bead read, while the second keeps relocated graph work
	// visible as agent activity. Both are computed after the cache-hit return.
