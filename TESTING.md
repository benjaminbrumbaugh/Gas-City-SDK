...
| --- | --- | --- | --- | --- | --- | --- |
| Audit baseline | all tracked test source | fixed_sleep: 473 calls / 173 files (historical regex census: 447 / 157) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Audit baseline | all tracked test source | subprocess: 655 calls / 191 files (historical regex census: 495 / 135) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Medium owner | `internal/runtime/acp` package `acp` | TestGenerationCleanupDoesNotUnlinkSuccessorSocket: net_listen | gc-k043y | ACP successor-socket generation cleanup is a checked Medium listener owner; both Unix stream listeners are confined to the generation-cleanup test and closed before test exit | P0.4c-listener | 2026-10-01 |
| Medium owner | `internal/runtime/acp` package `acp` | TestReservationLeaseClosesPreTransferLauncherCrashWindow: subprocess | gc-k043y | ACP inherited-lease crash-window proof is a checked Medium subprocess owner; the one long-lived child process is confined to the inherited-lease test and killed and waited by test cleanup | P0.4b | 2026-10-01 |
| Medium owner | `internal/runtime/acp` package `acp` | TestTransferSessionNameOwnerProtectsLiveChildAfterLauncherCrash: subprocess | gc-k043y | ACP child-ownership transfer is a checked Medium subprocess owner; the one long-lived child process is confined to the ownership-transfer test and killed and waited by test cleanup | P0.4b | 2026-10-01 |
...
| Medium owner | `internal/session` package `session` | TestProviderFenceAccountLockSerializesAcrossProcesses: subprocess | gc-k043y | provider-account cross-process exclusion is a checked Medium subprocess owner; the one re-executed test helper is confined to the cross-process lock test, released over a pipe, and synchronously waited | P0.4b | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | fixed_sleep: 318 calls / 123 files (historical regex census: 295 / 114) | ga-80po0c.2 | untagged fixed-sleep call/file totals cannot grow; reductions must lower this baseline; each owning test replaces elapsed wall time with its lifecycle signal | W1-W5 | 2026-10-01 |
| Source debt ratchet | all untagged test source | http_test_server: 324 calls / 68 files (historical regex census: 255 / 56) | ga-80po0c.2.2 | untagged HTTP test server call/file totals cannot grow; reductions must lower this baseline; each owning test closes its loopback server and removes duplicate server-backed coverage | P0.4c | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | subprocess: 442 calls / 129 files (historical regex census: 380 / 98) | ga-80po0c.2 | untagged subprocess call/file totals cannot grow; reductions must lower this baseline; each process-owning test removes or replaces its source call site | D1/D2/D5/D6/E6 | 2026-10-01 |
